...
/* "Recurrent pregnancy loss" — an "Educate IVF · Conditions" piece. Each section
 * is either a prose paragraph or a bulleted list, with an optional intro line.
 * All copy is localised via the ArticleRecurrentLoss namespace so it reads
...
export function RecurrentLossArticle({
  article,
  allArticles,
}: {
  article:     Article
  ...
  const t       = useTranslations('ArticleRecurrentLoss')
  ...
  const sections = t.raw('sections') as RPLSection[]
