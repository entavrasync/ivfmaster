...
/* "Age and IVF success" — an "Educate IVF · Understanding IVF" piece. Each section
 * is either a prose paragraph, a bulleted list, or a table. All copy is
 * localised via the ArticleAge namespace so it reads natively in English and Marathi. */
...
function DataTable({ table }: { table: NonNullable<AgeSection['table']> }) {
  return (
    <div style={{ overflowX: 'auto', margin: '1.5rem 0' }}>
      <table style={{
        ...
        fontFamily:      'var(--font-body)',
        fontSize:        'clamp(0.875rem, 0.25vw + 0.8125rem, 1rem)',
        textAlign:       'left',
      }}>
        <thead>
          <tr style={{ borderBottom: '2px solid rgba(28,42,72,0.1)' }}>
        ...
        <tbody>
          {table.rows.map((row, i) => (
            <tr
              key={i}
              style={{
                borderBottom: '1px solid rgba(28,42,72,0.06)',
                backgroundColor: i % 2 === 0 ? 'transparent' : 'rgba(28,42,72,0.02)',
              }}
            >
              {row.map((cell, j) => (
                <td
...
export function AgeArticle({
  ...
  const t       = useTranslations('ArticleAge')
  ...
  const sections = t.raw('sections') as AgeSection[]
